...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        return self.startingPosition, ()  # 奠定一个状态由状态本身以及从起始点到该状态的动作构成
    ...
    def getNextState(self, state, action):
        ...
        util.raiseNotDefined() # 注意这个地方可能会引起错误
...
def cornersHeuristic(state, problem):
    ...
    unvisit_corners = tuple(all_corners.difference(visited_corners))
    
    # 然后，我们计算从当前状态(state)到所有未访问过的终点之间的距离，这里使用DFS计算最佳
    def dfs_part_maze(point1, point2):
        x1, y1 = point1
        x2, y2 = point2
    ...
    distances = []
    for corner in unvisit_corners:
        value = dfs_part_maze(state[0], corner)
    ...
    return 0  # 此时是比较特殊的状态：终点都被访问过了，作为结束的标志
    
    return max_distance(unvisit)
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        #util.raiseNotDefined()
        return search.LikeAStarSearch(problem)
